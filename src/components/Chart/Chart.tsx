...
interface CustomTooltipProps {
    active?: boolean;
    payload?: Array<{ name?: string; value?: string | number, payload:{temperature:number,date:number}}>;
    celsius: boolean;
}
    ...
    if (active && payload && payload.length) {
        ...
        return (
            <div className={styles.customTooltip} style={{color: 'black'}}>
                {celsius &&
                    <span> {temperature > 0 ? '+' : ''}{celsius ? temperature.toString().slice(0, 4) : ((temperature * 9 / 5) + 32).toString().slice(0, 4)} &#8451;</span>}
                {!celsius &&
                    <span> {((temperature * 9 / 5) + 32) > 0 ? '+' : ''}{celsius ? temperature.toString().slice(0, 4) : ((temperature * 9 / 5) + 32).toString().slice(0, 4)} &#8457; </span>}
